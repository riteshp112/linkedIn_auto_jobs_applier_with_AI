...
class OpenAIModel(AIModel):
    def __init__(self, llm_model: str):
        ...
        self.model = G4FLLM(model=models.gpt_4o_mini)

    def invoke(self, prompt: str) -> AIMessage:
        print("invoke in openai")
        response = self.model.invoke(prompt)
        print("responseBefore", response)
        ...
        # Create an AIMessage with the content
        langchain_message = AIMessage(content=response)

        # Add metadata as attributes
        langchain_message.model_name = model_name
        langchain_message.system_fingerprint = system_fingerprint
        langchain_message.finish_reason = finish_reason
        print("responseAfter", langchain_message)
        # Currently the response is in text modify to form Langchain message containing content
...
class AIAdapter:
    ...
    def _create_model(self, config: dict, api_key: str) -> AIModel:
        ...
        print(
            "Using {0} with {1} from {2}".format(llm_model_type, llm_model, llm_api_url)
        )
